...
library(dplyr)
library(viridis)
setwd("/home/suzanne/rna_dohmain/09-urease/08-diff-abundance")
# add in annotations
...
HUU
```R
resLFC <- read.table("deseq_results_ure-HvD-HUU.txt")
# add in annotations
homd <- read.table("~/rna_dohmain/denovo/annotations.denovo.txt", header=T, sep="\t", quote="")
# filter by locus tag 
resdf <- as.data.frame(resLFC)
...
HEU
```R
resLFC <- read.table("deseq_results_ure-HvD-HEU.txt")
# add in annotations
homd <- read.table("~/rna_dohmain/denovo/annotations.denovo.txt", header=T, sep="\t", quote="")
# filter by locus tag 
resdf <- as.data.frame(resLFC)
...
HI
```R
resLFC <- read.table("deseq_results_ure-HvD-HI.txt")
# add in annotations
homd <- read.table("~/rna_dohmain/denovo/annotations.denovo.txt", header=T, sep="\t", quote="")
# filter by locus tag 
resdf <- as.data.frame(resLFC)
...
# subset for clarity
all <- all %>% select('baseMean', 'genus', 'species', "denovo")
HI <- HI %>% select('baseMean', 'genus', 'species', "denovo")
HEU <- HEU %>% select('baseMean', 'genus', 'species', "denovo")
HUU <- HUU %>% select('baseMean', 'genus', 'species', "denovo")

# group and get average base mean
group_all <- all %>% group_by(genus, species, denovo) %>% summarise(average_baseMean = mean(baseMean))
group_HI <- HI %>% group_by(genus, species, denovo) %>% summarise(average_baseMean = mean(baseMean))
group_HEU <- HEU %>% group_by(genus, species, denovo) %>% summarise(average_baseMean = mean(baseMean))
group_HUU <- HUU %>% group_by(genus, species, denovo) %>% summarise(average_baseMean = mean(baseMean))
...
df$Species_nested <- paste(df$genus, df$species, df$denovo, sep = "_")

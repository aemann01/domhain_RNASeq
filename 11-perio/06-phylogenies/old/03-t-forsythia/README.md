...
```sh
grep "forsythia" ~/rna_dohmain/homd_map/annotations.merge.txt | grep Tannerella  | grep -v none > tforsythia.annotations.txt
cat <(head -n 1 ~/rna_dohmain/homd_map/annotations.merge.txt) tforsythia.annotations.txt > temp
mv temp tforsythia.annotations.txt
parallel -a <(awk '{print $1}' tforsythia.annotations.txt | sed '1 i\Geneid') -j 60 -k "grep -w '{}' ../../homd_map/read_counts.txt" > tforsythia_counts.txt
```
...
#load data
setwd("/home/suzanne/rna_dohmain/11-perio/03-t-forsythia")
...
summary(res)
# [1] "number of genes with adjusted p value lower than 0.05:  2194"
# out of 11458 with nonzero total read count
# adjusted p-value < 0.05
# LFC > 0 (up)       : 511, 4.5%
# LFC < 0 (down)     : 1683, 15%
# outliers [1]       : 0, 0%
# low counts [2]     : 4887, 43%
...
summary(resLFC)
# [1] "number of genes with adjusted p value lower than 0.05:  2115"
# out of 11458 with nonzero total read count
# adjusted p-value < 0.1
# LFC > 0 (up)       : 1385, 12%
# LFC < 0 (down)     : 1570, 14%
# outliers [1]       : 0, 0%
# low counts [2]     : 3776, 33%
# (mean count < 2)
write.table(resLFC, file="deseq_results_tfor-HUUvHI.txt", quote=F, sep="\t")
save.image("deseq_results_tfor-HUUvHI.RData")
```
Valcona Plot
```R
load("deseq_results_tfor-HUUvHI.RData")
# add in annotations
...
pdf("volcano-HUUvHI.tforsythia.pdf", width=15, height=10)
overall_plot
dev.off()
system("~/.iterm2/imgcat ./volcano-HUUvHI.tforsythia.pdf")
```
Make beta diversity plot
```R
vld <- varianceStabilizingTransformation(se_star)
pdf("pca_HUUvHI.tforsythia.pdf")
plotPCA(vld, intgroup=c("hiv_status")) + theme_minimal()
dev.off()
system("~/.iterm2/imgcat ./pca_HUUvHI.tforsythia.pdf")
```
...
# long_df <- long_df %>% filter(Var1 == "susB" | Var1 == "kly" | Var1 == "eno" )
...
paste("number of genes with adjusted p value lower than 0.05: ", sum(res$padj < 0.05, na.rm=TRUE))
summary(res)
# [1] "number of genes with adjusted p value lower than 0.05:  1407"
# out of 11458 with nonzero total read count
# adjusted p-value < 0.05
# LFC > 0 (up)       : 397, 3.5%
# LFC < 0 (down)     : 1010, 8.8%
# outliers [1]       : 0, 0%
# low counts [2]     : 3332, 29%
# (mean count < 2)
...
paste("number of genes with adjusted p value lower than 0.05: ", sum(resLFC$padj < 0.05, na.rm=TRUE))
summary(resLFC)
# [1] "number of genes with adjusted p value lower than 0.05:  1356"
# out of 11458 with nonzero total read count
# adjusted p-value < 0.1
# LFC > 0 (up)       : 705, 6.2%
# LFC < 0 (down)     : 1358, 12%
# outliers [1]       : 0, 0%
# low counts [2]     : 2888, 25%
# (mean count < 2)
write.table(resLFC, file="deseq_results_tfor-HEUvHI.txt", quote=F, sep="\t")
save.image("deseq_results_tfor-HEUvHI.RData")
```
Valcona Plot
```R
load("deseq_results_tfor-HEUvHI.RData")
# add in annotations
homd <- read.table("tforsythia.annotations.txt", header=T, sep="\t", quote="")
# filter by locus tag 
resdf <- as.data.frame(resLFC)

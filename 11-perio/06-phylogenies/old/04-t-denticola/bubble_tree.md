...
library(viridis
library(reshape2)
load("deseq_results_tdent-HUUvHI.RData")
...
long_sub <- long_df %>% filter(Var1 == "oppA" | Var1 == "flaA" | Var1 == "flaB"| Var1 == "fliE" | Var1 == "cheX" | Var1 == "cheY" | Var1 == "hbpA" | Var1 == "hbpB" | Var1 == "troA")

```
HEU
```R
resLFC <- read.table("deseq_results_tdent-HEUvHI.txt")
# add in annotations
homd <- read.table("./tdent.annotations.txt", header=T, sep="\t", quote="")
# filter by locus tag 
...
dfHI <- subset(sortdf, log2FoldChange < 0)
dfHEU <- subset(sortdf, log2FoldChange > 0)
```
HI
```R

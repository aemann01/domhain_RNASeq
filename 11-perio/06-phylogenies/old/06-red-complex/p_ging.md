# Find which samples have more than rel abudannce 1% of P. gingivalis
```R
library(phyloseq)
library(ggplot2)
library(tidyverse)
...
genecounts <- read.table("../02-p-gingivalis/pging_counts.txt", header=T, sep="\t", row.names=1)
...
summary(res)
# [1] "number of genes with adjusted p value lower than 0.05:  6337"
# out of 26924 with nonzero total read count
# adjusted p-value < 0.05
# LFC > 0 (up)       : 2435, 9%
# LFC < 0 (down)     : 3902, 14%
# outliers [1]       : 0, 0%
# low counts [2]     : 15658, 58%
# (mean count < 2)
...
Valcona Plot
```R
load("deseq_results_pging-HEUvHI.RData")
# add in annotations
...
res_sub <- res_ord %>% filter(gene == "kgp" | gene == "rgpB" | gene == "rgpA" | gene == "hagA" | gene == "fimA" | gene== "serC" | gene =="folD" | gene== "fhs" | gene =="sda")
sigloc %>% filter(gene =="fhs")
#Create volcano plot
...
pdf("pca_HEUvHI.pging.pdf")
plotPCA(vld, intgroup=c("hiv_status")) + theme_minimal()
dev.off()
system("~/.iterm2/imgcat ./pca_HEUvHI.pging.pdf")
```

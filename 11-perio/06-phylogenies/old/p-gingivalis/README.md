# 1. Get reads from P. gingivalis
```sh
grep "gingivalis" ~/rna_dohmain/homd_map/annotations.merge.txt | grep Porphyromonas  | grep -v none > pging.annotations.txt
cat <(head -n 1 ~/rna_dohmain/homd_map/annotations.merge.txt) pging.annotations.txt > temp
mv temp pging.annotations.txt
parallel -a <(awk '{print $1}' pging.annotations.txt | sed '1 i\Geneid') -j 7 -k "grep -w '{}' ../../homd_map/read_counts.txt" > pging_counts.txt
```
...
summary(res)
# [1] "number of genes with adjusted p value lower than 0.05:  7405"

# out of 74749 with nonzero total read count
# adjusted p-value < 0.05
# LFC > 0 (up)       : 1535, 2.1%
# LFC < 0 (down)     : 5870, 7.9%
# outliers [1]       : 0, 0%
# low counts [2]     : 56040, 75%
# (mean count < 1)
...
# low counts [2]     : 56040, 75%
# (mean count < 1)
write.table(resLFC, file="deseq_results_pging-HEUvHI.txt", quote=F, sep="\t")
...
res_sub <- res_ord %>% filter(gene == "kgp" | gene == "rgpB" | gene == "rgpA" | gene == "hagA" | gene == "fimA")
...
pdf("volcano-HEUvHI.pging.pdf", width=15, height=10)
overall_plot
dev.off()
system("~/.iterm2/imgcat ./volcano-HEUvHI.pging.pdf")
```
...
# plot
pdf("p_ging.dna.pdf", width =15, heigh =10)
ggplot(data)+
...
dev.off()
system("~/.iterm2/imgcat ./p_ging.dna.pdf")
...
```
```R
actino <- subset_taxa(rel, V8=="Porphyromonas_gingivalis" | V8=="Tannerella_forsythia" | V8=="Treponema_denticola" | V8=="Fusobacterium_nucleatum")
glom <- tax_glom(actino, taxrank=rank_names(actino)[7])
data <- psmelt(glom) # create dataframe from phyloseq object
data
data$Sample<- factor(data$Sample,levels=unique(data$Sample))
# plot
pdf("red_complex.dna.pdf", width =15, heigh =10)
ggplot(data)+
  geom_bar(aes(x=Sample, y=Abundance,fill=V8),stat="identity", position="stack")+
  facet_grid(~ hiv_status, switch = "x", scales = "free_x") +
  theme_minimal()+
  ggplot2::theme(axis.text.x = element_text(color = "black", size = 4, angle = 90, hjust = .5, vjust = .5, face = "plain"),
  	  ...
  	  legend.text = element_text(size = 7),legend.title = element_text(size = 10))
dev.off()
system("~/.iterm2/imgcat ./red_complex.dna.pdf")
test <- data %>% group_by(Sample) %>% summarise(average_baseMean = across(c(Abundance)))
glom <- tax_glom(actino, taxrank=rank_names(actino)[6])
data <- psmelt(glom) # create dataframe from phyloseq object
mean(data$Abundance)
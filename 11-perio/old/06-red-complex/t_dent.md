# Find which samples have more than rel abudannce 1% of T. denticola
```R
library(phyloseq)
library(ggplot2)
library(tidyverse)
setwd("~/rna_dohmain/11-perio/06-red-complex")
#get relative abundance of t. denticola
...
```
# 2, Run DESeq HI vs HEU for samples that have T. denticola present
```R
...
genecounts <- read.table("../04-t-denticola/tdent_counts.txt", header=T, sep="\t", row.names=1)
...
summary(res)
# [1] "number of genes with adjusted p value lower than 0.05:  736"
# out of 17015 with nonzero total read count
# adjusted p-value < 0.05
# LFC > 0 (up)       : 345, 2%
# LFC < 0 (down)     : 391, 2.3%
# outliers [1]       : 0, 0%
# low counts [2]     : 4948, 29%
# (mean count < 3)
...
summary(resLFC)
# "number of genes with adjusted p value lower than 0.05:  682"
# out of 17015 with nonzero total read count
# adjusted p-value < 0.1
# LFC > 0 (up)       : 550, 3.2%
# LFC < 0 (down)     : 715, 4.2%
# outliers [1]       : 0, 0%
# low counts [2]     : 4289, 25%
# (mean count < 3)
...
# add in annotations
homd <- read.table("../04-t-denticola/tdent.annotations.txt", header=T, sep="\t", quote="")
# filter by locus tag 
...
colnames(rows) <- c('seq')
homd <- read.table("../03-t-denticola/tdent.annotations.txt", header=T, sep="\t", quote="")
#get the names
homd$Gene <- gsub(x = homd$gene, pattern = "_[0-9]", replacement = "")
homd$Gene <- gsub(x = homd$Gene, pattern = "[0-9]", replacement = "")
rows2 <-left_join(rows, homd, by = join_by(seq ==  locus_tag))

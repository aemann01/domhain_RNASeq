...
```
Get corresponding uniprot ids: hillary go process and stella is go_fucntion in the script
```sh
...
parallel -a <(awk '{print $2}' go_reads.txt | grep -v no_term) -j 50 -k "grep -wm 1 '{}' gene2go.sub"> go_info 
...
mv temp read_counts_go.txt
```
# 2. Run deseq for function go (on stella)
## HI vs HUU
```R
...
genes2go <- genes2go_added[, c("GO_ID", "Geneid_base")]
...
intersection <- intersect(row.names(filtered_resLFC), genes2go$gene)
...
# lfc_vector <- filtered_resLFC$log2FoldChange
# names(lfc_vector) <- row.names(filtered_resLFC)
